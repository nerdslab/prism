...
M4_PRED_LENGTH_MAP = {
    ...
    "Q": 8,
    "M": 18,
    "ME": 18,  # Month-end alias, equivalent to M
    "W": 13,
    "D": 14,
    "H": 48,
    "h": 48,  # Lowercase version, pandas newer version returns 'h' instead of 'H'
}

PRED_LENGTH_MAP = {
    "M": 12,
    "ME": 12,  # Add ME key, corresponds to original M
    "W": 8,
    "D": 30,
    "H": 48,
    "h": 48,  # Lowercase version, pandas newer version returns 'h' instead of 'H'
    "T": 48,
    "min": 48,  # Add min key, corresponds to original T
    "S": 60,
}
...
class Dataset:
    ...
    def prediction_length(self) -> int:
        ...
        # Compatible with pandas/gluonts old and new frequency aliases:
        # - 'h' is equivalent to 'H';
        # - 'min' is equivalent to 'T';
        # - 'QE' (Quarter End like 'QE-DEC') is treated as 'Q';
        # - 'YE' (Year End like 'YE-DEC') is treated as 'A';
        # - 'ME' remains month-end, same as 'M' which we have already mapped.
        if freq.lower() == "min":
            freq = "T"
        # Normalize quarter-end alias to 'Q'
        if freq.upper().startswith("QE"):
            freq = "Q"
        if freq.upper().startswith("YE"):
            freq = "A"
        if freq.upper() == "Y":
            freq = "A"
        # Dictionary already contains lowercase version (e.g., 'h'), so can use directly
        pred_map = M4_PRED_LENGTH_MAP if "m4" in self.name else PRED_LENGTH_MAP
        if freq not in pred_map:
            # If frequency not in dictionary, try converting to uppercase (compatible with old versions)
            freq = freq.upper()
        pred_len = pred_map[freq]
